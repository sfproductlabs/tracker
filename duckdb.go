...
func (i *DuckService) serve(w *http.ResponseWriter, r *http.Request, s *ServiceArgs) error {
	...
	case SVC_POST_AGREE:
		...
		if len(body) > 0 {
			...
			if err := json.Unmarshal(body, &b); err == nil {
				...
				_, err := i.Session.Exec(`INSERT INTO agreements (
				...
				_, err = i.Session.Exec(`INSERT INTO agreed (
					vid, created, cflags, sid, uid, avid, hhash, app, rel,
					url, ip, iphash, gaid, idfa, msid, fbid, country, region, culture,
					source, medium, campaign, term, ref, rcode, aff, browser, bhash, device,
					os, tz, latlon, zip, owner, org
	...
	case SVC_GET_AGREE:
		...
		rows, err := i.Session.Query("SELECT * FROM agreements WHERE vid = ?", vid)
		if err != nil {
			return err
		}
		defer rows.Close()
	...
	case SVC_GET_JURISDICTIONS:
		rows, err := i.Session.Query("SELECT * FROM jurisdictions")
		if err != nil {
			return err
		}
		defer rows.Close()
	...
	case SVC_GET_REDIRECTS:
		...
		rows, err := i.Session.Query("SELECT * FROM redirect_history")
		if err != nil {
			return err
		}
		defer rows.Close()
	...
	case SVC_GET_REDIRECT:
		var redirect string
		err := i.Session.QueryRow("SELECT urlto FROM redirects WHERE urlfrom = ?", fmt.Sprintf("%s%s", r.Host, r.URL.Path)).Scan(&redirect)
		if err == nil {
	...
	case SVC_POST_REDIRECT:
		...
		_, err = i.Session.Exec(`INSERT INTO redirects (urlfrom, urlto, created) VALUES (?, ?, ?)`,
			redirect["from"], redirect["to"], created)
		if err != nil {
			return err
		}

		_, err = i.Session.Exec(`INSERT INTO redirect_history (urlfrom, urlto, created) VALUES (?, ?, ?)`,
			redirect["from"], redirect["to"], created)
		if err != nil {
			return err
		}
