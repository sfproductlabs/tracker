...
# 1 million record insert to REPLICATED events table
# Tests performance with replication overhead

echo "🚀 FAST 1M RECORD INSERT - EVENTS TABLE (REPLICATED)"
echo "===================================================="
echo ""
...
echo "- Table: sfpla.events (ReplicatedReplacingMergeTree)"
echo ""
...
# Generate and insert in batches - write to temp file
TEMP_SQL=$(mktemp)
...
for ((batch=1; batch<=NUM_INSERTS; batch++)); do
    ...
    cat > "$TEMP_SQL" << EOF
INSERT INTO sfpla.events (tid, vid, uid, oid, etyp, ename, org, created_at) VALUES
EOF
...
echo "events:           $initial_events → $final_events (+$events_written)"
echo ""
